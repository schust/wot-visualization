...
def pt_average(db):
    ...
    for tank_id, tank in tanks.iterrows():
        ...
        merged = pd.concat([tank_winrates['tank_winrate'], player_winrates['player_winrate']], axis=1) \
            .reset_index(drop=True)

        total_players = merged.shape[0]

        plot(merged, current_tank, tank_id, tank, total_players)


def plot(data, current_tank, tank_id, tank, total_players):
    plt.clf()
    plt.plot(data['player_winrate'], data['tank_winrate'], linestyle='None', marker='.', alpha=0.1)
    axes = plt.gca()
    ...
    plt.savefig('plots/pt_scatter/' + plot_name)
